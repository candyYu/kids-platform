...
- UTF-8 文本；`#` 注释行、空行忽略。
- 按学科分块：学科行（语文 / 数学 / 英语 / 小提琴，可独占一行或同行带内容）
  开始一块，后续行直到下一学科行都属于该科（老师真实作业即此形态）。
- 空学科块跳过（警告）；纯空文件允许上传，表示"今天没有作业"。
- 语文块内容匹配正则 /摘星卡第?\s*(\d+)\s*课/ → 附 🎴 深链接
  （"摘星本 P1" 是纸质本页码，不匹配）。
- 上传脚本本地预解析，游离内容（学科行之外的行）拒绝上传并报行号。
- 解析逻辑在脚本与 web 客户端两处实现，必须保持一致。
